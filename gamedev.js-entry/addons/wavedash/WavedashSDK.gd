...
func _create_js_callback(req_id: int) -> JavaScriptObject:
	var cb = JavaScriptBridge.create_callback(func(args):
		...
		if response == null:
			response = {"success": false, "data": null, "message": "Failed to parse response: %s" % response_json}
...
func _create_js_error_callback(req_id: int) -> JavaScriptObject:
	var cb = JavaScriptBridge.create_callback(func(args):
		var err_msg: String = ""
		if args.size() > 0 and args[0]:
			err_msg = str(args[0])
		if err_msg == "":
			err_msg = "Promise rejected (unknown error)"
		_log("JS promise rejected (req %d): %s" % [req_id, err_msg])
		_pending_results[req_id] = {"success": false, "data": null, "message": err_msg}
		_request_resolved.emit(req_id)
	)
	_active_callbacks[-req_id] = cb  # negative key so it doesn't overwrite the .then() callback
	return cb

func _invoke_js(js_promise):
	_next_request_id += 1
	var req_id = _next_request_id
	var then_cb = _create_js_callback(req_id)
	var catch_cb = _create_js_error_callback(req_id)
	# Store the callbacks on window so they're accessible from JS eval
	var then_key = "_wd_then_%d" % req_id
	var catch_key = "_wd_catch_%d" % req_id
	var promise_key = "_wd_promise_%d" % req_id
	# Attach callbacks + promise to window
	var win = JavaScriptBridge.get_interface("window")
	win[then_key] = then_cb
	win[catch_key] = catch_cb
	win[promise_key] = js_promise
	# Chain .then/.catch entirely in JS so the promise resolves natively.
	# Cleanup happens INSIDE the callbacks so refs stay alive until settlement.
	JavaScriptBridge.eval("""
		(function() {
			...
			if (p && typeof p.then === 'function') {
				...
				window[cKey]('Not a promise');
				cleanup();
			}
		})();
	""".strip_edges() % [promise_key, then_key, catch_key, promise_key])
